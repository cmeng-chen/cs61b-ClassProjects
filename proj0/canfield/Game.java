...
class Game {
    ...
    /** Recording an arraylist of previous games. */
    private ArrayList<Game> _previousGames = new ArrayList<Game>();

    /** Recording the index of the previous
     *  game in the arraylist _previousGame. */
    private int pregame = -1;

    /** Undo the previous movement. */
    void undo() {
        if (pregame >= 0) {
            copyFrom(_previousGames.get(pregame));
            _previousGames.remove(pregame);
            pregame -= 1;
        }
    }
    ...
    /** Saves the games into the arraylist _previousGames.*/
    void saveGames() {
        Game newgame = new Game(this);
        _previousGames.add(newgame);
        pregame += 1;
    }
